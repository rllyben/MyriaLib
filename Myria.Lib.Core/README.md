...
If you're building a networked game and want to guarantee clients can't gain an advantage from gameplay mods, call `ModLoader.ApplyMultiplayerMode(true)` before connecting — *gameplay* mods, and specifically a *gameplay* mod's `plugin.dll`, are skipped so the client loads the same unmodified data as the server. A **visual-only** mod's `plugin.dll` still loads even in multiplayer (only its content files are restricted to visual paths — its code isn't sandboxed or restricted at all). `ModLoader.GetModInfo()` reports the client's active mods (including a SHA-256 fingerprint per gameplay mod) if your server wants to allowlist specific mods instead of blocking all of them.
...
**Security note:** a `plugin.dll` runs as fully-trusted, unsandboxed .NET code — `ModLoader` doesn't (and, on modern .NET, can't easily) restrict what it does. This is the same trade-off any DLL-moddable game makes; it's not specific to multiplayer, and `ApplyMultiplayerMode` doesn't fully close it (see above). `ModLoader.Load()` logs a `GameLog.Info` entry every time a `plugin.dll` is actually loaded, naming the mod, so a host UI that surfaces `GameLog` shows players exactly when third-party code is about to run — but the underlying trust decision (only install mods from sources you trust) is unavoidably the player's to make, the same as installing any other third-party executable.
